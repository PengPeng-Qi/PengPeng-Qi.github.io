...
## BOM
### BOM简介
BOM（浏览器对象模型），提供了独特于内容而于浏览器窗口进行相互的对象，其核心对象是window。  
...
`DOMContentLoaded` 事件触发时，仅**当DOM加载完成**，不包括样式表，图片，flash等等。
```js
...
当鼠标移动到元素上时就会触发`mouseenter`事件，类似`mouseover`，它们两者之间的差别是，`mouseover`鼠标经过自身盒子会触发，经过**子盒子还会触发**。`mouseenter`只会经过**自身盒子触发**，之所以这样，就是因为`mouseenter`不会冒泡，跟`mouseenter`搭配鼠标离开`mouseleave`同样不会冒泡。  
### 动画函数封装
